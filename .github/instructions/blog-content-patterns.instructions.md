...
- `thumb` is generated by `pnpm thumbs:generate` and should be `'./thumb.webp'`. Add to frontmatter after generating.
...
3. Run `pnpm thumbs:generate` to create `thumb.webp`
4. Add `thumb: './thumb.webp'` to frontmatter
5. `pnpm dev` will automatically copy thumbs on startup
